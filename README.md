...
Utility to create persistent and nested lay-outs in next-js (without the app-directory).<br/>
Default in next-js, Layouts would re-mount on each navigation. <br/>
Next's `app`-directory has better support for persisting (nested) layouts, but a lot of the next/react-ecosystem still revolves around the `pages`-directory.
